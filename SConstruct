...
if OSPosix:
	...
	params.Add( PathVariable( 'path_bin', 'Path to the DMO Tools binaries',
		'${execdir}', PathVariable.PathAccept ) )
	
elif OSWindows:
	params.Add( PathVariable( 'path_bin', 'Path to the DMO Tools Installation',
		'${PROGRAMFILES}\DMO Tools', PathVariable.PathAccept ) )
...
if OSPosix:
	...
	parent_report[ 'dmo tools path' ] = parent_env.subst( parent_env[ 'path_bin' ] )
